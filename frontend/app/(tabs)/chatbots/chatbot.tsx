  ...
  return (
    <View className="flex-1 bg-[#F6F9FF] dark:bg-gray-800 pt-12">
      ...
      <View className="flex-row items-center px-4 py-3 gap-2">
        <TouchableOpacity onPress={() => router.push('/chatbots/camera')} className="p-2 rounded-full bg-white dark:bg-gray-900 border border-lime-500">
          <MaterialCommunityIcons name="line-scan" size={20} color="#7ED321" />
        </TouchableOpacity>
        <TextInput
          placeholder="Ketik disini..."
          value={input}
          onChangeText={setInput}
          className="flex-1 bg-white dark:bg-gray-900 rounded-full px-4 py-2 border border-lime-500"
          {...(colorScheme === 'dark' ? { placeholderTextColor: "#fff" } : { placeholderTextColor: "#000" } )}
        />
        <TouchableOpacity onPress={sendMessage} className="p-2">
          <FontAwesome name="send" size={20} color="#7ED321" />
        </TouchableOpacity>
      </View>
    </View>
  );
